...
import com.shch.lz.ssm.db.DataSourceEnum;
import com.shch.lz.ssm.db.DynamicDataSource;
import com.shch.lz.ssm.util.SpringContextUtil;
...
/**
 * Created by Link at 13:18 on 3/29/18.
 */
// slim this class Todo
public abstract class BaseServiceImpl<Mapper, Record, Example> implements BaseService<Record, Example> {
    ...
    public List<Record> selectByExampleWithBLOBsForStartPage(Example example, Integer pageNum, Integer pageSize) {
        try {
            DynamicDataSource.setDataSource(DataSourceEnum.SLAVE.getName());
            Method selectByExmapleWithBLOBsForStartPage = mapper.getClass().getDeclaredMethod("selectByExampleWithBLOBsForStartPage", example.getClass());
            PageHelper.startPage(pageNum, pageSize, false);
            Object result = selectByExmapleWithBLOBsForStartPage.invoke(mapper, example);
            return (List<Record>) result;
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
    ...
    public Record selectFirstByExample(Example example) {
        try {
            DynamicDataSource.setDataSource(DataSourceEnum.SLAVE.getName());
            Method selectFirstByExample = mapper.getClass().getDeclaredMethod("selectByExample", example.getClass());
            List<Record> result = (List<Record>) selectFirstByExample.invoke(mapper, example);
    ...
    public Record selectFirstByExampleWithBLOBs(Example example) {
        try {
            ...
            List<Record> result = (List<Record>) selectFirstByExampleWithBLOBs.invoke(mapper, example);
            if (null != result && result.size() > 0) {
                return result.get(0);
            }
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
